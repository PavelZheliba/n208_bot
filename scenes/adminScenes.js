...
// При входе в сцену просто показываем главное меню админки
adminScene.enter(async (ctx) => {
    const keyboard = Markup.inlineKeyboard([
        [Markup.button.callback('🗂️ Управление категориями', 'manage_categories')],
        [Markup.button.callback('➕ Добавить товар (прямое)', 'add_product')],
        [Markup.button.callback('🏠 Выйти из админ-панели', 'exit_admin')]
    ]);
    await ctx.reply('Добро пожаловать в админ-панель!', keyboard);
});


// Кнопка "Добавить товар" запускает визард добавления товара
adminScene.action('add_product', async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.scene.enter('ADD_PRODUCT_WIZARD');
});

// Кнопка "Выход" просто покидает сцену админки
adminScene.action('exit_admin', async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.reply('Вы вышли из админ-панели.');
    await ctx.scene.leave();
});

// Любой текст в админ-меню будет встречен этим сообщением
